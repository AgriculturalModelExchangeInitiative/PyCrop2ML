...
class Java_Cyml_ast():
    ...
    def visit_bool(self, node):
        if "value" in dir(node) and "name" in dir(node): 
            val = self.visit(node.value)
            return  {'type': 'boolean', 'name': str(node.name), 'value': val["value"].capitalize() if "value" in val else val.capitalize(), 'pseudo_type': 'boolean'}
        elif "value" in dir(node): 
            val = self.visit(node.value)
            return {'type': 'boolean', 'value': val.capitalize(), 'pseudo_type': 'boolean'}
        elif "name" in dir(node): 
            return {'type': 'boolean', 'name':str(node.name), 'pseudo_type': 'boolean'}
    ...
    def visit_array(self, node):
        if "dim" not in dir(node): node.dim = 0
