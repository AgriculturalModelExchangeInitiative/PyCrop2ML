...
gaiMean_ = 0.0
countGaiMean = 0
for i in range(0 , len(listTTShootWindowForPTQ1_t1) - 1 + 1 , 1):
    TTList.append(listTTShootWindowForPTQ1_t1[i + 1 - 1])
    GAIList.append(listGAITTWindowForPTQ_t1[i + 1 - 1])
...
for i in range(count , len(TTList) - 1 + 1 , 1):
    listTTShootWindowForPTQ1.append(TTList[i + 1 - 1])
    listGAITTWindowForPTQ.append(GAIList[i + 1 - 1])
for i in range(0 , len(listGAITTWindowForPTQ) - 1 + 1 , 1):
    gaiMean_ = gaiMean_ + listGAITTWindowForPTQ[i + 1 - 1]
