...
def simulation(datafile, vardata, params, init):
    ...
    #outputs
    output_names = ["SoilSurfaceTemperature","SnowIsolationIndex","SnowWaterContent","SoilTempArray"]
    ...
    for i in range(0,len(df.index)-1):
        ...
        SoilSurfaceTemperature,SnowIsolationIndex,SnowWaterContent,SoilTempArray= SoilTemperatureComponent.model_soiltemperature(cCarbonContent,iAirTemperatureMax,iAirTemperatureMin,iGlobalSolarRadiation,iRAIN,iCropResidues,iPotentialSoilEvaporation,iLeafAreaIndex,SoilTempArray,cSoilLayerDepth,cFirstDayMeanTemp,cAverageGroundTemperature,cAverageBulkDensity,cDampingDepth,iSoilWaterContent)

        df_out.loc[i] = [SoilSurfaceTemperature,SnowIsolationIndex,SnowWaterContent,SoilTempArray]
