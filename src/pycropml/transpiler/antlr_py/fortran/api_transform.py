...
from pycropml.transpiler.antlr_py.fortran.builtin_typed_api import builtin_type_check
...
class StandardCall(Standard):
    ...
    def expand(self, args):
        if not self.expander:
            q = builtin_type_check(self.namespace, self.function, args[0], args)[-1]
            return {'type': 'standard_call', 'namespace': self.namespace, 'function': self.function, 'args': args[0], 'pseudo_type': q}
        else:
...
class StandardMethodCall(Standard):
    ...
    def expand(self, args):
        ...
        if not self.expander:
            q = builtin_type_check(self.type, self.message, args[0], args=args[0])[-1]
            return {'type': 'standard_method_call', 'receiver': args[0][0], 'message': self.message, 'args': [], 'pseudo_type': q}
        else:
...
def len_expander(type, message, args):
    receiver_type = args[0][0]['pseudo_type']
    ...
    if message == 'len' and 'special' in args[0]: # len(sys.argv)
        ...
        if a =="str" and message =="float" and args[0][0]["value"]==b"nan":
            return {"type":'notAnumber', 'value' : 'nan', 'pseudo_type':'float'}
        else: 
            return {'type': 'standard_method_call', 'receiver':args[0][0], 'args': [], 'message': message, 'pseudo_type': q[-1]}

def min_expander(type, message, args):
    if len(args[0])==1:
        return {'type': 'standard_call', 'namespace': 'system', 'function': 'min', 'args': args, 'pseudo_type': args[0][0]["pseudo_type"][1]}
    else:
        for i in range(len(args[0])-1):
            if args[0][i]["pseudo_type"]!=args[0][i+1]["pseudo_type"]:
                pseudo = "float"
                break
            else: pseudo = args[0][i]["pseudo_type"]
        return {'type': 'standard_call', 'namespace': 'system', 'function': 'min', 'args': args[0], 'pseudo_type': pseudo}
        
def max_expander(type, message, args):
    if len(args[0])==1:
        return {'type': 'standard_call', 'namespace': 'system', 'function': 'max', 'args': args, 'pseudo_type': args[0][0]["pseudo_type"][1]}
    else:
        for i in range(len(args[0])-1):
            if args[0][i]["pseudo_type"]!=args[0][i+1]["pseudo_type"]:
                pseudo = "float"
        ...
        return {'type': 'standard_call', 'namespace': 'system', 'function': 'max', 'args': args[0], 'pseudo_type':pseudo}
...
def any_expander(type, message, args) :
    return {'type': 'standard_method_call',
      'receiver': args[0][0]["left"],
      'message': 'contains?',
      'args': [args[0][0]["right"]],
      'pseudo_type': 'bool'} 

def all_expander(type, message, args) :
    return {'type': 'standard_method_call',
      'receiver': args[0][0]["left"],
...
def indice_expander(type, message, args):
    return {'type': 'standard_method_call',
      'receiver': args[0][0],
      'message': 'index',
      'args': [args[0][1]],
...
def add_expander(type, message, args):
    return {'type': 'ExprStatNode',
...
SYSTEM_API = {
        ...
        'list_sub':
                    {'indice': StandardMethodCall('list', 'indice', expander=indice_expander),
                    'add': StandardMethodCall('list', 'add', expander=add_expander)
                    }
}

FUNCTION_API = {
        'any' :     StandardMethodCall('list', 'any', expander=any_expander),
        'all' :     StandardMethodCall('list', 'all', expander=all_expander),
