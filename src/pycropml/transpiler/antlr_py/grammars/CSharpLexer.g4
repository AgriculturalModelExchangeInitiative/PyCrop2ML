...
INTERPOLATED_REGULAR_STRING_START:
    '$"' { self.OnInterpolatedRegularStringStart(); } -> pushMode(INTERPOLATION_STRING)
;
INTERPOLATED_VERBATIUM_STRING_START:
    '$@"' { self.OnInterpolatedVerbatiumStringStart(); } -> pushMode(INTERPOLATION_STRING)
;
...
DOUBLE_CURLY_INSIDE           : '{{';
OPEN_BRACE_INSIDE             : '{' { self.OpenBraceInside(); } -> skip, pushMode(DEFAULT_MODE);
...
CLOSE_BRACE_INSIDE        : '}'  { self.OnCloseBraceInside(); } -> skip, popMode;
FORMAT_STRING             : ~'}'+;
