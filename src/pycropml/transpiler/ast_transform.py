...
class AstTransformer():
    ...
    def visit_top_level(self, nodes):
        ...
        if isinstance(nodes, ExprNodes.ImportNode):
            ...
            self.type_env.top['_%s' % module_name], self.type_env.top[module_name] = self.type_env.top[module_name], 'library'
            return {"type": "import",
                    "module": module_name,
                    }    
            ...
            else: name_list = [name.value for name in nodes.module.name_list.args]
            ...
            self.type_env.top['functions'][nodes.name] = ['Function'] + ([None] * len(nodes.args)) + [None]
    ...
    def visit_node(self, node):
        ...
        if isinstance(node, Nodes.FromImportStatNode):
            ...
            fields = {field: getattr(node, field) for field in node.child_attrs}
            if "self" in fields: fields.pop("self")
    ...
    def visit_singleassignmentnode(self,node, lhs, rhs,location):
        ...
        if isinstance(lhs, ExprNodes.NameNode) and  not isinstance(rhs, ExprNodes.ImportNode):
            name = lhs.name
            e = self.type_env.top[name]  
                                 
            if e is None:   
                self.notdeclared(name, location[0])     
            elif e:
                if e in ("list","dict","tuple", "array"):
                    a=self._compatible_types(e, value_node['type'], "can't change the type of variable %s in %s " % (name, self.function_name))
                else:
                    a=self._compatible_types(e, value_node['pseudo_type'], "can't change the type of variable %s in %s at %s " % (name, self.function_name, location[0]))
            return {
                    'type': 'assignment',
                    'target': {
                    'type': 'local',
        ...
        if isinstance(lhs, ExprNodes.TupleNode):
            if not isinstance(rhs, ExprNodes.TupleNode):
            ...
            for t, child in zip(lhs.args, rhs.args):
                ...
                rights.append(self.visit_singleassignmentnode(node,t, child_node, location))
    ...
    def visit_inplaceassignmentnode(self, node, lhs, rhs, location):
        z = node.end_pos()
        newNode = ExprNodes.BinopNode(pos=z,operand1=lhs,operand2=rhs,operator=node.operator)
    ...
    def visit_binopnode(self,node, operand1, operand2, location):
        op = node.operator
        left_node, right_node = self.visit_node(operand1), self.visit_node(operand2)
    ...
    def visit_indexnode(self, node, base,index, location):
        ...
        else: value_general_type = self._general_type(value_node['pseudo_type'])
        ...
        if isinstance(index, Nodes.Node):
            ...
            if value_general_type in ['str', 'list', 'tuple'] and z['pseudo_type'] != 'int':
                raise PseudoCythonTypeCheckError('a non int index for %s %s' % (value_general_type, z['pseudo_type']))

            if value_general_type == 'dict' and z['pseudo_type'] != value_node['pseudo_type'][1]:
                raise PseudoCythonTypeCheckError('a non %s index for %s %s' % (value_node['pseudo_type'][1], value_general_type, z['pseudo_type']))
    ...
    def visit_tuplenode(self, node, args, mult_factor, location):
        ...
        element_nodes, accidentaly_homogeneous, element_type = self.visit_elements(args, 'tuple', homogeneous=False)      
        return {
    ...
    def visit_dictnode(self, node, key_value_pairs, location):
        ...
        pairs = [{'type': 'pair', 'key': self.visit_node(key_value_pairs[0].key), 'value': self.visit_node(key_value_pairs[0].value)}]
        key_type, value_type = pairs[0]['key']['pseudo_type'], pairs[0]['value']['pseudo_type']
        for a, b in zip(key_value_pairs[1:], key_value_pairs[1:]):
            pairs.append({'type': 'pair', 'key': self.visit_node(a.key), 'value': self.visit_node(b.value)})
    ...
    def visit_simplecallnode(self, node,function, coerced_self, args, arg_tuple, location):
        ...
        if isinstance(function, ExprNodes.NameNode) and function.name in BUILTIN_FUNCTIONS:
            ...
            if function.name in list(self.type_env.top['functions'].keys()):
                ...
                if function.name not in meth :
                    ...
                    if self.retrieve_library(function.name) not in self._imports:
                        self._imports.append(self.retrieve_library(function.name))
            ...
            elif self._general_type(value_node['pseudo_type']) in PSEUDON_BUILTIN_TYPES: # [2].append
    ...
    def _translate_builtin_method_call(self, class_type, base, message, args, location):
        ...
        if not api:
            raise translation_error(
                "pseudo-cython doesn\'t support %s#%s"  % (serialize_type(class_type), message),
                location, self.lines[location[0]],
    ...
    def visit_condexprnode(self,node,test,true_val,false_val, location):
        ...
        a=self._compatible_types(val1["pseudo_type"], val2["pseudo_type"], "can't change the type of variable")        
        return {
            'type': 'cond_expr_node',
    ...
    def visit_defnode(self,node,args,star_arg, starstar_arg,decorators, body, return_type_annotation, location):
        ...
        arg_nodes = [arg if not isinstance(arg, Nodes.Node) else self.visit_node(arg) for arg in args]
        env=arg_nodes
    ...
    def visit_cargdeclnode(self,node, base_type,declarator,default,annotation, location):
        name = declarator.base.name if isinstance(declarator, Nodes.CArrayDeclaratorNode) else declarator.name
        decl = {"name":name , "type": "local"}
        if default and not isinstance(default,ExprNodes.ListNode) :
            de = self.visit_node(default)
            decl["value"]=default.value
            decl["pseudo_type"]=de["pseudo_type"]
            a=self._compatible_types(base_type.name, decl["pseudo_type"], "can't change the type of variable %s in %s " % (base_type.name, self.function_name))
            ...
            if isinstance(de.dimension, ExprNodes.NameNode) or  isinstance(de.dimension, ExprNodes.IntNode):
                ...
                for d in self.visit_node(de.dimension.args):
                    elts.append(d)
            dim = len(elts)
            decl = {"name":de.base.name, "type": "local", "dim":dim, "elts":elts, "pseudo_type":["array", base_type.name]}
            self.type_env.top[de.base.name]= decl["pseudo_type"]      
        else:
            decl["pseudo_type"]=self.visit_node(base_type.name)
        self.type_env.top[name]= decl["pseudo_type"]       
            ...
            if not isinstance(de, Nodes.CArrayDeclaratorNode):
                ...
                if isinstance(de.dimension, ExprNodes.NameNode) or  isinstance(de.dimension, ExprNodes.IntNode):
                    ...
                    for d in self.visit_node(de.dimension.args):
                        elts.append(d)
                dim = len(elts)
                decl = {"name":de.base.name, "type": "array", "dim":dim, "elts":elts, "pseudo_type":["array", base_type.name]}
                self.type_env.top[de.base.name]= decl["pseudo_type"]
    ...
    def _translate_iter(self,target, k):
        if isinstance(k, ExprNodes.SimpleCallNode) and isinstance(k.function, ExprNodes.NameNode):
            if k.function.name == 'enumerate':
                if len(k.args) != 1 or not isinstance(target, ExprNodes.TupleNode) or len(target.args) != 2:
                    raise PseudoCythonTypeCheckError('enumerate expected one arg not %d and two indices' % len(k.args))
                ...
                if isinstance(target, ExprNodes.NameNode):
                    ...
                    raise PseudoCythonTypeCheckError('range expected two indices')
                elif not k.args or len(k.args) > 3:
                    raise PseudoCythonTypeCheckError('range expected 1 to 3 args not %d' % len(k.args))
                ...
                if len(k.args) < 2 or not isinstance(target, ExprNodes.TupleNode) or len(k.args) != len(target.args):
                    raise PseudoCythonTypeCheckError('zip expected 2 or more args and the same number of indices not %d' % len(k.args))
        ...
        if isinstance(target,ExprNodes.TupleNode):
            raise PseudoCythonNotTranslatableError("pseudo doesn't support tuples yet")

        elif not isinstance(target, ExprNodes.NameNode):
            raise PseudoCythonNotTranslatableError("pseudo doesn't support %s as an iterator" % sequence_node['type'])
    ...
    def _translate_enumerate(self, targets, sequence):
        ...
        if not isinstance(targets[0], ExprNodes.NameNode) or not isinstance(targets[1], ExprNodes.NameNode):
            raise PseudoCythonTypeCheckError('expected a name for an index not %s' % type(targets[0]).__name__)
    ...
    def _confirm_iterable(self, sequence_type):
        ...
        if sequence_general_type not in ITERABLE_TYPES:
            raise PseudoCythonTypeCheckError('expected an iterable type, not %s' % serialize_type(sequence_type))

    def _translate_range(self, targets, range):
        if len(range) == 1:
            start, end, step = {'type': 'int', 'value': 0, 'pseudo_type': 'int'}, self.visit_node(range[0]), {'type': 'int', 'value': 1, 'pseudo_type': 'int'}
        elif len(range) == 2:
            start, end, step = self.visit_node(range[0]), self.visit_node(range[1]), {'type': 'int', 'value': 1, 'pseudo_type': 'int'}
        else:
            start, end, step = tuple(map(self.visit_node, range[:3]))
        for label, r in [('start', start), ('end', end), ('step', step)]:
            if r['pseudo_type'] != 'int':
                raise PseudoCythonTypeCheckError('expected int for range %s index' % label)

        if not isinstance(targets[0], ExprNodes.NameNode):
            raise PseudoCythonTypeCheckError('index is not a name %s' % type(targets[0]).__name__)
        return {
            'type': '_range',
            'start': start,
            'end': end,
    ...
    def visit_sliceindexnode (self, node, start, stop,base,slice, location):
        ...
        if stop:
            ...
            self._confirm_index(upper_node['pseudo_type'], 'int', getattr(stop, 'location', location), 'slice index')
        if start:
            lower_node = self.visit_node(start)
            self._confirm_index(lower_node['pseudo_type'], 'int', getattr(start, 'location', location), 'slice index')
        if stop and start:
            name = base
    ...
    def _compatible_types(self, from_, to, err, silent=False):
        if from_[0]=="array": from_[0]="list"
        if from_=="unknown" or to=="unknown":
            return to
        elif isinstance(from_, str) or isinstance(from_, EncodedString)  :
            if not isinstance(to, str) and not isinstance(to, EncodedString):
                if silent:
                    return False
                else:
                    raise PseudoCythonTypeCheckError(err + ' from %s to %s' % (serialize_type(from_), serialize_type(to)))
    ...
    def assert_translatable(self, node, **pairs):
        for label, (expected, actual) in pairs.items():
            if actual != expected:
                raise PseudoCythonNotTranslatableError("%s in %s is not a part of pseudo-translatable cython" % (label if label[-1] != '_' else label[:-1], node))
