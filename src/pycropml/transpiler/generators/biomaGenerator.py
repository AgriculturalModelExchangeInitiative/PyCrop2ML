...
class BiomaGenerator(CsharpGenerator):
    ...
    BioMA components and use the NodeVisitor to generate a csharp
...
""")
    ...
    def desc(self,node, n, inp, vartype) :
        ...
        self.newline(node)
        self.write("pd%s.DomainClassType = typeof(%s%s.DomainClass.%s%s);"%(n,self.customer, self.name,self.name,inp.variablecategory.capitalize()))
        self.newline(node)
        self.write('pd%s.PropertyName = "%s";'%(n,inp.name if not inp.name.endswith("_t1") else inp.name[:-3]))
        self.newline(node)
        self.write('pd%s.PropertyType = (%s%s.DomainClass.%s%sVarInfo.%s).ValueType.TypeForCurrentValue;'%(n, self.customer,self.name,self.name, inp.variablecategory.capitalize(),inp.name if not inp.name.endswith("_t1") else inp.name[:-3]))
        self.newline(node)
        self.write('pd%s.PropertyVarInfo =(%s%s.DomainClass.%s%sVarInfo.%s);'%(n,self.customer,self.name,self.name, inp.variablecategory.capitalize(),inp.name if not inp.name.endswith("_t1") else inp.name[:-3]))
        self.newline(node)
    ...
    def getStrategyDomainClassesTypes(self, node):	
        ...
        self.open(node)
        self.write("return new List<Type>() {  typeof(%s%s.DomainClass.%sState),  typeof(%s%s.DomainClass.%sState), typeof(%s%s.DomainClass.%sRate), typeof(%s%s.DomainClass.%sAuxiliary), typeof(%s%s.DomainClass.%sExogenous)};"%(self.customer,self.name, self.name,self.customer,self.name, self.name,self.customer,self.name, self.name,self.customer,self.name, self.name,self.customer,self.name, self.name))
        self.close(node)
        self.newline(extra=1)
    ...
    def TestPostConditions(self, node):
        ...
        for out in self.model.outputs:
            self.newline(node)
            self.write("RangeBasedCondition r%s = new RangeBasedCondition(%s%s.DomainClass.%s%sVarInfo.%s);"%(n,self.customer,self.name, self.name, out.variablecategory.capitalize(),out.name)); self.newline(node)
            self.write("if(r%s.ApplicableVarInfoValueTypes.Contains( %s%s.DomainClass.%s%sVarInfo.%s.ValueType)){prc.AddCondition(r%s);}"%(n, self.customer, self.name, self.name, out.variablecategory.capitalize(), out.name, n)); self.newline(node)
            n = n+1
        ...
        self.open(node)
        self.write('string msg = "%s.%s, " + this.GetType().Name + ": Unhandled exception running post-condition test. ";'%(self.customer,self.name)); self.newline(node)
    ...
    def TestPreConditions(self, node):
        self.write("public string TestPreConditions(%s%s.DomainClass.%sState s,%s%s.DomainClass.%sState s1,%s%s.DomainClass.%sRate r,%s%s.DomainClass.%sAuxiliary a,%s%s.DomainClass.%sExogenous ex,string callID)"%(self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name))
        self.open(node)
        self.write("try")
        self.open(node)
        ...
        for inp in self.model.inputs:
            if "variablecategory" in dir(inp):
                self.newline(node)
                self.write('%s%s.DomainClass.%s%sVarInfo.%s.CurrentValue=%s.%s;'%(self.customer,self.name, self.name, inp.variablecategory.capitalize(),inp.name if not inp.name.endswith("_t1") else inp.name[:-3],category[inp.variablecategory], inp.name if not inp.name.endswith("_t1") else inp.name[:-3]))
            self.newline(node)
        self.write('ConditionsCollection prc = new ConditionsCollection();'); self.newline(node)
        self.write('Preconditions pre = new Preconditions(); ' ); self.newline(node)
        n =  1
        for inp in self.model.inputs:
            if "variablecategory" in dir(inp):
                self.newline(node)
                self.write("RangeBasedCondition r%s = new RangeBasedCondition(%s%s.DomainClass.%s%sVarInfo.%s);"%(n,self.customer,self.name, self.name,inp.variablecategory.capitalize(), inp.name if not inp.name.endswith("_t1") else inp.name[:-3])); self.newline(node)
                self.write("if(r%s.ApplicableVarInfoValueTypes.Contains( %s%s.DomainClass.%s%sVarInfo.%s.ValueType)){prc.AddCondition(r%s);}"%(n,self.customer, self.name, self.name, inp.variablecategory.capitalize(), inp.name if not inp.name.endswith("_t1") else inp.name[:-3], n)); self.newline(node)
                n = n+1
        self.newline(node)
        for p in self.model.parameters:
        ...
        self.write("catch (Exception exception)"); 
        self.open(node)
        self.write('string msg = "%s.%s, " + this.GetType().Name + ": Unhandled exception running pre-condition test. ";'%(self.customer,self.name)); self.newline(node)
        self.write('throw new Exception(msg, exception);'); self.newline(node)
        self.close(node)
        self.close(node)
        self.newline(extra=1)
    ...
    def estimate(self, node):
        self.write("public void Estimate(%s%s.DomainClass.%sState s,%s%s.DomainClass.%sState s1,%s%s.DomainClass.%sRate r,%s%s.DomainClass.%sAuxiliary a,%s%s.DomainClass.%sExogenous ex)"%(self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name))
        self.open(node)
        self.write("try")
        self.open(node)
        ...
        self.write("catch (Exception exception)")
        self.open(node)
        self.write('string msg = "Error in component %s%s, strategy: " + this.GetType().Name + ": Unhandled exception running model. "+exception.GetType().FullName+" - "+exception.Message;'%(self.customer,self.name))				
        self.newline(node)
    ...
    def visit_function_definition(self, node):      
        ...
        if (not node.name.startswith("model_") and not node.name.startswith("init_")) :
            ...
            self.write('%s%s.DomainClass.%sState s, %s%s.DomainClass.%sState s1, %s%s.DomainClass.%sRate r, %s%s.DomainClass.%sAuxiliary a, %s%s.DomainClass.%sExogenous ex)'%(self.customer,self.name, self.name,self.customer,self.name,self.name,self.customer,self.name, self.name,self.customer,self.name,self.name,self.customer, self.name,self.name))
            self.open(node)
    ...
    def visit_module(self, node):
        self.write("using %s%s.DomainClass;"%(self.customer,self.name))
        self.newline(node)
        self.write("namespace %s%s.Strategies"%(self.customer,self.name))
        self.open(node)
        self.write("public class %s : IStrategy%s%s"%(self.model.name, self.customer,self.name))
        self.open(node)
...
""")
    ...
    def generate(self, nodes, typ, name): 
        self.using()
        self.write("namespace %s%s.DomainClass"%(self.customer,name))
        self.open(nodes)
...
def to_struct_bioma(models, rep, name):
    generator = BiomaTrans(models)
...
""")
    ...
    def constructor(self, node):
        ...
        for m in self.model.model:
            name = m.name
            self.write("lAssStrat0_0.Add(typeof(%s%s.Strategies.%s).FullName);"%(self.customer,self.model.name, name))
            self.newline(1)
        self.write("mo0_0.AssociatedStrategies = lAssStrat0_0;")
        self.newline(1)
        self.write("_modellingOptionsManager = new ModellingOptionsManager(mo0_0);")
    ...
    def TestPostConditions(self, node):
        ...
        self.newline(node)
        for m in self.model.model:
            self.write('ret += _%s.TestPostConditions(s, s1, r, a, ex, " strategy %s%s.Strategies.%s");'%(m.name,self.customer, self.model.name, self.name ))
            self.newline(node)
        ...
        self.write("catch (Exception exception)"); 
        self.open(node)
        self.write('string msg = "Component %s.%s, " + this.GetType().Name + ": Unhandled exception running post-condition test. ";'%(self.customer,self.name)); self.newline(node)
        self.write('throw new Exception(msg, exception);'); self.newline(node)
        self.close(node)
        self.close(node)
        self.newline(extra=1)
    ...
    def TestPreConditions(self, node):
        ...
        self.newline(node)
        for m in self.model.model:
            self.write('ret += _%s.TestPreConditions(s, s1, r, a, ex, " strategy %s%s.Strategies.%s");'%(m.name,self.customer, self.model.name, self.name ))
            self.newline(node)
        ...
        self.write("catch (Exception exception)"); 
        self.open(node)
        self.write('string msg = "Component %s.%s, " + this.GetType().Name + ": Unhandled exception running pre-condition test. ";'%(self.customer,self.name)); self.newline(node)
        self.write('throw new Exception(msg, exception);'); self.newline(node)
        self.close(node)
        self.close(node)
        self.newline(extra=1)
    ...
    def calculateModel(self, node):
        self.write("private void CalculateModel(%s%s.DomainClass.%sState s,%s%s.DomainClass.%sState s1,%s%s.DomainClass.%sRate r,%s%s.DomainClass.%sAuxiliary a,%s%s.DomainClass.%sExogenous ex)"%(self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name))
        self.open(node)
    ...
    def visit_module(self, node): 
        ...
        self.newline(node)
        self.write("namespace %s%s.Strategies"%(self.customer,self.model.name))
        self.open(node)
        self.write("public class %sComponent : IStrategy%s%s"%(self.model.name, self.customer,self.model.name))
        self.open(node)
    ...
    def visit_function_definition(self, node):
        if node.name.startswith("model"):
            self.write("private void EstimateOfAssociatedClasses(%s%s.DomainClass.%sState s,%s%s.DomainClass.%sState s1,%s%s.DomainClass.%sRate r,%s%s.DomainClass.%sAuxiliary a,%s%s.DomainClass.%sExogenous ex)"%(self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name))
        else:
            self.write("public void Init(%s%s.DomainClass.%sState s,%s%s.DomainClass.%sState s1,%s%s.DomainClass.%sRate r,%s%s.DomainClass.%sAuxiliary a,%s%s.DomainClass.%sExogenous ex)"%(self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name,self.customer,self.name,self.name))
            self.init=True
        self.open(node)
    ...
    def interfaceStrategy(self, node):
        ...
        self.open(node)
        self.write('public interface IStrategy%s%s : IStrategy'%(self.customer,self.model.name))
        self.open(node)
...
def to_wrapper_bioma(models, rep, name, customer = ''):
    generator = BiomaCompo(model = models, customer=customer)
