MODULE Cumulttfrom_mod
    ...
    IMPLICIT NONE
    CONTAINS
        SUBROUTINE cumulttfrom_(calendarMoments,calendarCumuls,switchMaize,cumulTT,cumulTTFromZC_65,cumulTTFromZC_39,cumulTTFromZC_91)
            REAL, INTENT(OUT) :: cumulTTFromZC_65
            REAL, INTENT(OUT) :: cumulTTFromZC_39
            REAL, INTENT(OUT) :: cumulTTFromZC_91
            CHARACTER(65), ALLOCATABLE , DIMENSION(:), INTENT(IN) :: calendarMoments
    ...
    !                          - unit : °C d
            ...
            IF(ANY(calendarMoments .EQ. 'EndGrainFilling')) THEN
                IF((switchMaize .EQ. 0)) THEN
                    cumulTTFromZC_91 = cumulTT - calendarCumuls(indice(calendarMoments, 'FlagLeafLiguleJustVisible'))
                END IF
            END IF
